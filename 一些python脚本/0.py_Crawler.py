#!/usr/bin/env python3
import argparse, hashlib, re, signal, socket, imghdr, os, pickle, posixpath, time, numpy, cv2, io
import urllib.parse
...
图片爬虫
"""
output_path    = 'D:\shuju\pa\新村\测试图片\游商小贩'
search_strings = ['监控下扔垃圾','垃圾影响环境']
search_numbers = 30
...
def download(url: str, output_dir: str, limit: int):
    global tried_urls
    global image_md5s
    global in_progress
    ...
    try:
        ...
        if not imgtype:
            print('SKIP: Invalid image, not saving ' + str(len(tried_urls)) + ' --> ' + name)
            return
        ...
        # imagefile.close()
        im0 = numpy.array(Image.open(io.BytesIO(image)).convert('RGB'))
        im0 = cv2.cvtColor(im0, cv2.COLOR_RGB2BGR)
        im0_name = os.path.join(output_dir, filename)
        im0_name = os.path.splitext(im0_name)[0] + '.jpg'
        cv2.imencode('.jpg', im0)[1].tofile(im0_name)
        print(" OK : " + str(len(tried_urls)) + ' --> ' + filename)
        tried_urls.append(url)
    except Exception as e:
        print("FAIL: " + str(len(tried_urls)) + ' --> ' + name, str(e))

def fetch_images_from_keyword(keyword: str='', output_dir: str='', filters: str='', limit: int=300):
    global tried_urls
    global image_md5s
    global urlopenheader
    current = 1
    ...
    while True:
        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and len(tried_urls) >= limit:
                    return
                download(link, output_dir, limit)
                current += 1
            last = links[-1]
        except IndexError:
...
def main():
    ...
    parser.add_argument('-k', '--search_string', default=search_strings, type=list, nargs='+', help='Keyword to search')
    parser.add_argument('-f', '--search-file', action='store_true', help='use search-string as a path to a file containing search strings line by line', required=False)
    parser.add_argument('-o', '--output', default=output_path, help='Output directory', required=False)
    parser.add_argument('-a', '--adult-filter-off', help='Disable adult filter', action='store_true', required=False)
    parser.add_argument('--filters', help='Any query based filters you want to append when searching for images, e.g. +filterui:license-L1', default='', required=False)
    parser.add_argument('--limit', help='Make sure not to search for more than specified amount of images.', default=300, required=False, type=int)
    ...
    for search_string in args.search_string:
        if len(search_string) < 1:
            continue
        if args.output:
            output_dir = Path(args.output) / search_string
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
    
        signal.signal(signal.SIGINT, backup_history)
        try:
            download_history = open(os.path.join(args.output, 'download_history.pickle'), 'rb')
        ...
        fetch_images_from_keyword(search_string, output_dir, '', args.limit)
